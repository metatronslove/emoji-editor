...
try {
    $chessGame = new ChessGame($gameId);
    $result = $chessGame->makeMove($from, $to, $userId);

    if ($result['success']) {
        // Oyun sonu kontrolü
        $gameEnd = $chessGame->checkGameEnd();
        if ($gameEnd) {
            $result['game_end'] = $gameEnd;

            // Aktivite kaydı oluştur
            $gameData = GameCommon::getGameState($gameId);
            $winnerId = $gameEnd['winner'] === 'white' ? $gameData['player1_id'] : $gameData['player2_id'];
